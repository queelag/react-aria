import { ID, IDUtils, rc } from '@queelag/core'
...
export class TabberStore extends ComponentStore {
  ...
  handleKeyboardEvents = (event: KeyboardEvent): void => {
    ...
    switch (event.key) {
      ...
      case Key.SPACE:
        ...
        StoreLogger.verbose(this.id, 'handleKeyboardEvents', `The default event has been prevented and the propagation has been stopped.`)
  ...
  focusListItem(index: number): void {
    ...
    if (!ref) return StoreLogger.error(this.id, 'focusSelectedListItem', `Failed to find the list item ref with index ${index}.`)

    ref.current.focus()
    StoreLogger.debug(this.id, 'focusSelectedListItem', `The list item with index ${index} has been focused.`)
  }
  ...
  deleteListItemRef = (index: number): void => {
    this.listItemRefs.delete(index)
  ...
  get focusedListItemIndex(): number {
    ...
    if (items.length <= 0) return rc(() => StoreLogger.error(this.id, 'focusedListItemIndex', `There are no items.`), 0)

    index = items.findIndex((v: HTMLButtonElement) => v === document.activeElement)
    if (index < 0) return rc(() => StoreLogger.error(this.id, 'focusedListItemIndex', `Failed to find the focused list item index.`), 0)
  ...
  set listItemsLength(listItemsLength: number) {
    ...
    StoreLogger.verbose(this.id, 'setListItemsLength', `The list item ids and panel ids have been set based on length ${listItemsLength}.`)
