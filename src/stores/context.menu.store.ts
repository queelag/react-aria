...
class ContextMenuStore extends ComponentStore<HTMLDivElement> {
  ...
  setExpanded = (expanded: boolean): void => {
    ...
    Logger.debug(this.id, 'setExpanded', `The menu trigger has been ${expanded ? 'expanded' : 'collapsed'}.`)

    if (this.isCollapsed) {
      this.triggerRef.current.focus()
      Logger.debug(this.id, 'handleKeyboardInteractions', `The trigger has been focused.`)
    }
